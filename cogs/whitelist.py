...
class PlayerInfoCog(commands.Cog):
    ...
    async def kick_player(self, server_name, steamid, reason="not being whitelisted"):
        try:
            ...
            if "Failed" in result:
                print(
                    f"Failed to execute kick command for {identifier} on {server_name}: {result}"
                )
            else:
                print(
                    f"Successfully executed kick command for {identifier} on {server_name}: {result}"
                )
            ...
            if "CONNECTION_CHANNEL" in server_info:
                ...
                if channel:
                    embed_description = (
                        f"Player `{identifier}` kicked for {reason}."
                        if "Failed" not in result
                        else f"Failed to kick Player `{identifier}`: {reason}."
                    )
            ...
            print(
                f"Exception during kick command for {identifier} on {server_name}: {e}"
            )
                ...
                for line in data.split("\n")[1:]:
                    if line.strip():
                        ...
                        if self.is_valid_steamid(steamid):
                            existing_players[steamid] = {
                                "name": name,
                                "playeruid": playeruid,
                                "whitelist": existing_players.get(steamid, {}).get(
                                    "whitelist", False
                                ),
                            }
                file.seek(0)
    ...
    async def steamid_autocomplete(
    ...
    async def search(
        ...
        if player_info:
            ...
            embed.add_field(
                name="Name", value=f"```{player_info['name']}```", inline=False
            )
            embed.add_field(
                name="Player UID",
                value=f"```{player_info['playeruid']}```",
                inline=False,
            )
            embed.add_field(name="SteamID", value=f"```{steamid}```", inline=False)
            embed.add_field(
                name="Whitelist",
                value=f"```{str(player_info['whitelist'])}```",
                inline=False,
            )
            embed.set_footer(
    ...
    async def on_steamid_autocomplete(
        self, interaction: nextcord.Interaction, current: str
    ):
    ...
    async def name_autocomplete(self, interaction: nextcord.Interaction, current: str):
        ...
        matches = [
            ...
            if player["name"] and current.lower() in player["name"].lower()
        ]
        return matches[:25]
    ...
    async def searchname(
        ...
        if player_info and player_steamid:
            ...
            embed.add_field(
                name="SteamID", value=f"```{player_steamid}```", inline=False
            )
            embed.add_field(
                name="Whitelist",
                value=f"```{str(player_info['whitelist'])}```",
    ...
    async def whitelist_add(
        self,
        interaction: nextcord.Interaction,
        steamid: str = nextcord.SlashOption(description="Enter SteamID", required=True),
        playeruid: str = nextcord.SlashOption(
            description="Enter PlayerUID", required=False
        ),
    ):
        if not steamid and not playeruid:
            await interaction.response.send_message(
                "Please provide either a SteamID or PlayerUID.", ephemeral=True
            )
            return
        ...
        with open(self.player_data_file, "r+") as file:
            ...
            if identifier not in players:
                ...
                file.truncate()
                message = (
                    f"Player {identifier} added to whitelist and will be fully registered upon joining."
                    if steamid
                    else f"Player with PlayerUID {playeruid} added to whitelist and will be fully registered upon joining."
                )
            else:
                ...
                file.truncate()
                message = (
                    f"Player {identifier} added to whitelist."
                    if steamid
                    else f"Player with PlayerUID {playeruid} added to whitelist."
                )
    ...
    async def whitelist_remove(self, interaction: nextcord.Interaction, steamid: str):
        ...
        if steamid in players and players[steamid]["whitelist"]:
            ...
            with open(self.player_data_file, "w") as file:
                json.dump(players, file)
            await interaction.response.send_message(
                f"Player {steamid} removed from whitelist.", ephemeral=True
            )
        else:
            await interaction.response.send_message(
                f"Player {steamid} not found or not on whitelist.", ephemeral=True
            )
...
def setup(bot):
    ...
    bot.all_slash_commands.extend(
        [cog.search, cog.searchname, cog.whitelist_add, cog.whitelist_remove]
    )
